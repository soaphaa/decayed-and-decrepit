# Dead and Decayed

**Dead and Decayed** is a top-down zombie survival game set in the post-apocalyptic Paradise City. A viral outbreak has turned the population into zombies, and the remaining survivors are forced to hide underground. You play as a scavenger who was selected to go out of the bunker you've been in sheltering in for several months, venturing out to collect resources before your bunker runs out of supplies.

---

## Gameplay Features

- Player Movement & Combat
- **Enemies**
  - Zombie AI with pathfinding, chasing, and attack behavior
  - Zombies spawn progressively on each floor
  - Multiple bunker floors
  - Progressive difficulty scaling per floor (increasing zombie numbers)

---

## How to Play

1. Use **WASD** or arrow keys to move.
2. Aim with the mouse and attack zombies with **spacebar**.
3. Clear zombies to progress through bunker floors.
4. Survive and collect resources before supplies run out.

---

## Campfire Hackathon
A 24hr Hackathon Organized by Hack Club that I was able to attend! The theme was "Under the Surface" which was where the underground bunker and zombie apocolypse idea came from.

---

## Credits
All Art made by me and my other CF Hackathon team members
The name and backstory is inspired by the Minecraft Roleplay series, Decayed and Decrepit. Amazing series, go check it out!

- Game concept and design by *(Your Name / Team Name)*
- Graphics and sprites *(if applicable)*
- Zombie AI, game mechanics, and UI implemented in HTML/CSS/JS
